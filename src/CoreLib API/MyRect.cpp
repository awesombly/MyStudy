...
Point MyRect::getPoint() const noexcept
{
	return m_point;
}
...
Point MyRect::getCenter() const noexcept
{
	return {m_point.x + m_width / 2, m_point.y + m_height / 2 };
}
Point MyRect::getWH()	const noexcept
{
...
void MyRect::setWH(const Point& width_height)			noexcept
{
...
// radian to degree
float MyRect::getAngle(const int& sx, const int& sy, const int& dx, const int& dy)
{
	int&& vx = dx - sx;
	int&& vy = dy - sy;
...
// radian to degree
float MyRect::getAngle(const Point& sP, const Point& dP)
{
	float&& vx = dP.x - sP.x;
	float&& vy = dP.y - sP.y;
...
// degree to radian
float MyRect::getRadian(const float& angle)
{
	return angle * (PI / 180);
}