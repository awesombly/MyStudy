...
void MyUI::UpdateVertexList(ID3D11DeviceContext* pDContext, ID3D11Buffer** pVertexBuffer, vector<Vertex_PNCT>* pVertexList, const bool& useListUpdate)
{
	if (useListUpdate)
	{
		m_proCoord.x = (float)getX() / Window::getClientRect().right  * 2.0f - 1;
		...
		(*pVertexList)[3].pos = { m_proCoord.z, m_proCoord.w, 0.0f };
	}
	pDContext->UpdateSubresource(*pVertexBuffer, 0, nullptr, &(*pVertexList).at(0), 0, 0);
}

void MyUI::UpdateVertexList(ID3D11DeviceContext* pDContext, ID3D11Buffer** pVertexBuffer, vector<Vertex_PNCT>* pVertexList, const MyRect& rect, const bool& useListUpdate)
{
	if (useListUpdate)
	{
...
void MyUI::UpdateVertexList(ID3D11DeviceContext* pDContext, ID3D11Buffer** pVertexBuffer, vector<Vertex_PNCT>* pVertexList, const MyVector4& vector4, const bool& useListUpdate)
{
	if (useListUpdate)
	{
...
HRESULT MyUI::CreateVertexBuffer(ID3D11Device* pDevice, ID3D11Buffer** pVertexBuffer, vector<Vertex_PNCT>* pVertexList) const
{
	D3D11_BUFFER_DESC bufferDesc = { 0, };

	bufferDesc.Usage = D3D11_USAGE_DEFAULT;				// 사용 모드(CPU->GPU 접근제어)
	bufferDesc.ByteWidth = (UINT)(pVertexList->size() * sizeof(Vertex_PNCT));	// 정점크기 * 갯수
...
void MyUI::SetColor(const float& red, const float& green, const float& blue, vector<Vertex_PNCT>* pVertexList)
{
