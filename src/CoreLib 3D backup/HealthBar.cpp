...
bool HealthBar::Init() noexcept
{
	...
	CreateVertexBuffer(DxManager::GetDevice(), &m_pFrontBuffer, &m_vertexList);
	// 중간
	...
	UpdateVertexList(DxManager::GetDContext(), &m_pFrontBuffer, &m_FrontVertexList, true);
...
void HealthBar::setHealthRate(const float& healthRate)
{
	...
	m_FrontVertexList[3].tex = { posVector.z, posVector.w };
	...
	m_FrontVertexList[3].pos = { m_FrontVertexList[1].pos.x, m_BackVertexList[5].pos.y, 0.0f };
}
...
void HealthBar::UpdateInterBuffer()
{
	...
	m_vertexList[3].pos = { m_vertexList[1].pos.x, m_BackVertexList[5].pos.y, 0.0f };
