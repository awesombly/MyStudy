...
bool WriteManager::CreateDeviceResource(IDXGISurface1* pSurface)
{
	...
	if (HRESULT hr; FAILED(hr = m_pD2dFactory->CreateDxgiSurfaceRenderTarget(pSurface, (const D2D1_RENDER_TARGET_PROPERTIES *)&props, &m_pRT)))
		return false;
	if (m_pRT == nullptr)
	{
		wostringstream ostr;
		ostr << __FUNCTION__ << ", RenderTarget nullptr!" << endl;
...
void WriteManager::DrawText(const MyRect& rect, const wstring_view& text)
{
	D2D1_RECT_F layoutRect = D2D1::RectF(
		rect.getLeft() / m_DPIScale.x,
		rect.getTop() / m_DPIScale.y,
		rect.getRight() / m_DPIScale.x,
		rect.getBottom() / m_DPIScale.y);
...
void WriteManager::SetText(const D2D1_POINT_2F& layoutSize, const wstring_view& text, const D2D1::ColorF& color, const float& fontSize, const wstring_view& fontFamily)
{
	...
	{
		wostringstream ostr;
		ostr << __FUNCTION__ << ", TextLayout Error!" << endl;
		OutputDebugString(ostr.str().c_str());
	}
	...
	{
		wostringstream ostr;
		ostr << __FUNCTION__ << ",	LayoutSetting Error!" << endl;
		OutputDebugString(ostr.str().c_str());
	}
