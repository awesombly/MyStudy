...
bool MyPlayer::MouseCheck()
{
	...
	if (Input::isDebug)
	{
		...
		/*if (Input::Instance->getMouseState(2) == EKeyState::DOWN)
		{
			...
			ostr.str(L"");
			ostr << L"게임좌표 : x(" << m_prevCursor.x + Window::DrawPoint.x << "), y(" << m_prevCursor.y << ")" << endl;
		...
		else if (Input::Instance->getMouseState(2) == EKeyState::UP)
		{
			static wostringstream ostr;
			ostr.str(L"");
			ostr << L"너비높이 : width(" << Input::Instance->getCursor().x - m_prevCursor.x + Window::DrawPoint.x
				<< "), height(" << Input::Instance->getCursor().y - m_prevCursor.y << ")" << endl;
			OutputDebugString(ostr.str().c_str());
		}*/
	}
	// 벽 생성
	...
	{
		ObjectManager::Instance->TakeObject(L"ItemIcon", getCenter())->Init();
	}
	return true;
}
...
void MyPlayer::RemoveUsingItem(Item* pItem)
{
	...
	{
		wostringstream ostr;
		ostr << __FUNCTION__ << L", " << pItem->m_myName << L", 아이템 찾기 실패!" << endl;
		OutputDebugString(ostr.str().c_str());
		return;
	}
