...
bool IntroScene::Init() noexcept
{
	...
	m_ObjectManager.PushObject(m_HealthBar = new HealthBar(L"HpBar", EObjType::UI))
		->setRect(getClientRect().right * 0.67f, getClientRect().bottom * 0.05f, getClientRect().right * 0.3f, getClientRect().bottom * 0.07f);
	m_HealthBar->Init();
	m_HealthBar->setEnable(false);
	// 혈압
	m_ObjectManager.PushObject(m_BloodRate = new BloodRate(L"BloodRate", EObjType::UI))
		->setRect(getClientRect().right * 0.675f, getClientRect().bottom * 0.13f, getClientRect().right * 0.29f, getClientRect().bottom * 0.035f);
	m_BloodRate->Init(m_DxManager.m_pD3dContext);
...
bool IntroScene::Render() noexcept
{
	...
	if (loadingTime < 5.0f)
	{
		...
		m_loadingBitmap->Render(m_DxManager.m_pD3dContext);
	}
	else
	{
